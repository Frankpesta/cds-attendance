...
import { createCdsGroupAction, deleteCdsGroupAction } from "@/app/actions/cds_groups";
...
export default function GroupsPage() {
  ...
  const create = async () => {
    ...
    try {
      const res = await createCdsGroupAction({
        name: form.name,
      ...
      if (res.ok) {
        ...
        queryClient.invalidateQueries({ queryKey: ["cds-groups"] });
      } else {
        push({ variant: "error", title: "Create failed", description: res.error });
      }
    } catch (e: unknown) {
  ...
  const handleDelete = async (g: { id: string; name: string }) => {
    ...
    try {
      const res = await deleteCdsGroupAction(g.id);
      if (res.ok) {
        push({ variant: "success", title: "Group deleted", description: `${g.name} has been deleted successfully` });
        queryClient.invalidateQueries({ queryKey: ["cds-groups"] });
      } else {
        push({ variant: "error", title: "Delete failed", description: res.error });
      }
    } catch (e: unknown) {
  ...
  const meetingDays = (g: { meeting_days?: unknown }) => {
    const d = g.meeting_days;
    ...
    <div className="space-y-6">
      ...
      <Card>
        <CardHeader>
          ...
          <p className="text-sm text-muted-foreground">{groupList.length} groups found</p>
        </CardHeader>
        <CardContent>
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {groupList.map((g: { id: string; name: string; meeting_days?: unknown; meeting_time?: string; venue_name?: string }) => (
              <div key={g.id} className="p-4 border rounded-lg hover:shadow-md transition-shadow">
